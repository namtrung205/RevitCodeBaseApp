...
sealed partial class Build
{
  Target PublishGitHub =>
    _ => _
      ...
      .Executes( async () =>
      {
        ...
        var newRelease = new NewRelease( Version ) { Name = Version, Body = changelog, TargetCommitish = GitRepository.Commit } ;
  ...
  static async Task UploadArtifactsAsync( Release release, IEnumerable<string> artifacts )
  {
    foreach ( var file in artifacts ) {
      ...
      {
        ContentType = "application/x-binary", FileName = Path.GetFileName( file ), RawData = File.OpenRead( file )
      } ;
